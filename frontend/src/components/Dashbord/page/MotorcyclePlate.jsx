...
const MotorcyclePlate = () => {
  ...
  const fetchPlates = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/plates_motorcycle`);
  ...
  const handleAddPlate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/addPlate/plates_motorcycle`, {
        method: "POST",
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updateStatus/plates_motorcycle/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeletePlate = async (id, plate) => {
    ...
    if (window.confirm(`คุณแน่ใจหรือไม่ที่จะลบรายการนี้ ${plate}? `)) {
      try {
        const response = await fetch(
          `${API_URL}/deletePlate/plates_motorcycle/${id}`,
          { method: "DELETE" }
        );

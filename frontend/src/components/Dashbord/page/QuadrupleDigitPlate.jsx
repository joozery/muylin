...
const QuadrupleDigitPlate = () => {
  ...
  const fetchPlates = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/plates_quadruple_digit`);
  ...
  const handleAddPlate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/addPlate/plates_quadruple_digit`,
        {
          method: "POST",
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updateStatus/plates_quadruple_digit/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeletePlate = async (id, plate) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const response = await fetch(
          `${API_URL}/deletePlate/plates_quadruple_digit/${id}`,
          { method: "DELETE" }
        );
  ...
  const handleEdit = async (formModal) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updatePlate/plates_quadruple_digit/${formModal.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="p-4 bg-white rounded shadow">
      <h2 className="text-xl font-bold mb-4 font-['Prompt']">ทะเบียนเลขโฟร์</h2>
...
export default QuadrupleDigitPlate;

...
// 🔹 ฟังก์ชันคำนวณค่าผลรวมจากหมายเลขทะเบียน
const charValueMap = {
  'ก': 1, 'ด': 1, 'ถ': 1, 'ท': 1, 'ภ': 1,
  'ข': 2, 'บ': 2, 'ป': 2, 'ง': 2, 'ช': 2,
  'ต': 3, 'ฑ': 3, 'ฒ': 3, 'ฆ': 3,
...
const NonAuction = () => {
  ...
  const handleAddPlate = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/addPlate_old`, {
        method: "POST",
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const response = await fetch(`${API_URL}/updateStatus_old/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeletePlate = async (id) => {
    try {
      const response = await fetch(`${API_URL}/deletePlate_old/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="p-4 bg-white rounded shadow">
      <h2 className="text-xl font-bold mb-4 font-['Prompt']">ป้ายขาวดำหมวดเก่า/ไม่ประมูล</h2>

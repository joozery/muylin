...
const SpecialPlate = () => {
  ...
  const handleAddPlate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/addPlate/plates_special`, {
        method: "POST",
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updateStatus/plates_special/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeletePlate = async (id, plate) => {
    ...
    if (window.confirm(`คุณแน่ใจหรือไม่ที่จะลบรายการนี้ ${plate}? `)) {
      try {
        const response = await fetch(
          `${API_URL}/deletePlate/plates_special/${id}`,
          { method: "DELETE" }
        );
...
const formatNumber = (value) => {
  ...
  const handleEdit = async (formModal) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updatePlate/plates_special/${formModal.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },

...
const GoldGraphicPlate = () => {
  ...
  const fetchPlates = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/plates_gold_graphic`);
  ...
  const handleAddPlate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/addPlate/plates_gold_graphic`, {
        method: "POST",
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updateStatus/plates_gold_graphic/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeletePlate = async (id, plate) => {
    ...
    if (window.confirm(`คุณแน่ใจหรือไม่ที่จะลบรายการนี้ ${plate}? `)) {
      try {
        const response = await fetch(
          `${API_URL}/deletePlate/plates_gold_graphic/${id}`,
          { method: "DELETE" }
        );
  ...
  //ModalEdit
      ...
      const handleEdit = async (formModal) => {
        ...
        try {
          const response = await fetch(
            `${API_URL}/updatePlate/plates_gold_graphic/${formModal.id}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
...
export default GoldGraphicPlate;

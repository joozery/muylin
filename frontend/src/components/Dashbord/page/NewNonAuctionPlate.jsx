...
const NewNonAuctionPlate = () => {
  ...
  const fetchPlates = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/plates_new_non_auction`);
  ...
  const handleAddPlate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/addPlate/plates_new_non_auction`, {
        method: "POST",
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updateStatus/plates_new_non_auction/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeletePlate = async (id, plate) => {
    ...
    if (window.confirm(`คุณแน่ใจหรือไม่ที่จะลบรายการนี้ ${plate}? `)) {
      try {
        const response = await fetch(
          `${API_URL}/deletePlate/plates_new_non_auction/${id}`,
          { method: "DELETE" }
        );
  ...
  //ModalEdit
      ...
      const handleEdit = async (formModal) => {
        ...
        try {
          const response = await fetch(
            `${API_URL}/updatePlate/plates_new_non_auction/${formModal.id}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
...
export default NewNonAuctionPlate;

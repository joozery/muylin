...
const DoubleDigitPlate = () => {
  ...
  const fetchPlates = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/plates_double_digit`);
  ...
  const handleAddPlate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/addPlate/plates_double_digit`, {
        method: "POST",
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/updateStatus/plates_double_digit/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeletePlate = async (id, plate) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const response = await fetch(
          `${API_URL}/deletePlate/plates_double_digit/${id}`,
          { method: "DELETE" }
        );
...
export default DoubleDigitPlate;

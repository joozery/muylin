...
const PlatesQuadrupleDigit = ({ cover, url }) => {
  const [platesQuadrupleDigit, setPlatesQuadrupleDigit] = useState([]);

  // ✅ ดึงข้อมูลทะเบียนรถเลขโฟร์จาก API
  useEffect(() => {
    const fetchPlatesQuadrupleDigit = async () => {
      try {
        const response = await fetch(`${url}/plates_quadruple_digit`);
        if (!response.ok) throw new Error("Failed to fetch plates");
        const data = await response.json();
        setPlatesQuadrupleDigit(data);
      } catch (error) {
        console.error("❌ Error fetching plates_quadruple_digit:", error);
      }
    };
    fetchPlatesQuadrupleDigit();
  }, [url]);
  ...
  return (
    <>
      ...
      <div className="text-center mt-12 mb-6"> {/* เพิ่ม mt-12 เพื่อให้มีระยะห่างด้านบน */}
        <h2 className="text-2xl font-bold italic">ทะเบียนรถเลขโฟร์</h2>
      </div>

      {/* ✅ แสดงข้อมูลทะเบียนรถเลขโฟร์ */}
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
        {platesQuadrupleDigit.length > 0 ? (
          platesQuadrupleDigit.map((plate, index) => (
            <div
              key={index}
...
export default PlatesQuadrupleDigit;
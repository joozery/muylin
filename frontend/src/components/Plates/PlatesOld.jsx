...
const PlatesOld = ({ cover, url }) => {
  const [platesOld, setPlatesOld] = useState([]);

  // ✅ ดึงข้อมูลทะเบียนรถขาวดำหมวดเก่าจาก API
  useEffect(() => {
    const fetchPlatesOld = async () => {
      try {
        const response = await fetch(`${url}/plates_old`);
        if (!response.ok) throw new Error("Failed to fetch plates");
        const data = await response.json();
        setPlatesOld(data);
      } catch (error) {
        console.error("❌ Error fetching plates_old:", error);
      }
    };
    fetchPlatesOld();
  }, [url]);

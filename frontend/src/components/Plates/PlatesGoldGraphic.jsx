...
const PlatesGoldGraphic = ({ cover, url }) => {
  const [platesGoldGraphic, setPlatesGoldGraphic] = useState([]);

  // ✅ ดึงข้อมูลทะเบียนรถระฆังทอง (กราฟฟิคสีทอง) จาก API
  useEffect(() => {
    const fetchPlatesGoldGraphic = async () => {
      try {
        const response = await fetch(`${url}/plates_gold_graphic`);
        if (!response.ok) throw new Error("Failed to fetch plates");
        const data = await response.json();
        setPlatesGoldGraphic(data);
      } catch (error) {
        console.error("❌ Error fetching plates_gold_graphic:", error);
      }
    };
    fetchPlatesGoldGraphic();
  }, [url]);
  ...
  return (
    <>
      ...
      <div className="text-center mt-12 mb-6">
        <h2 className="text-2xl font-bold italic">ทะเบียนรถระฆังทอง (กราฟฟิคสีทอง)</h2>
      </div>

      {/* ✅ แสดงข้อมูลทะเบียนรถระฆังทอง (กราฟฟิคสีทอง) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
        {platesGoldGraphic.length > 0 ? (
          platesGoldGraphic.map((plate, index) => (
            <div
              key={index}
          ...
          <p className="text-center text-gray-400 col-span-4">
            ❌ ไม่พบทะเบียนรถระฆังทอง (กราฟฟิคสีทอง)
          </p>
        )}
      </div>
    </>
...
export default PlatesGoldGraphic;
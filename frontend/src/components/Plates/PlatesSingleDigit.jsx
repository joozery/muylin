...
const PlatesSingleDigit = ({ cover, url }) => {
  const [platesSingleDigit, setPlatesSingleDigit] = useState([]);

  // ✅ ดึงข้อมูลทะเบียนรถเลขตัวเดียวจาก API
  useEffect(() => {
    const fetchPlatesSingleDigit = async () => {
      try {
        const response = await fetch(`${url}/plates_single_digit`);
        if (!response.ok) throw new Error("Failed to fetch plates");
        const data = await response.json();
        setPlatesSingleDigit(data);
      } catch (error) {
        console.error("❌ Error fetching plates_single_digit:", error);
      }
    };
    fetchPlatesSingleDigit();
  }, [url]);
  ...
  return (
    <>
      ...
      <div className="text-center mt-12 mb-6"> {/* เพิ่ม mt-12 เพื่อให้มีระยะห่างด้านบน */}
        <h2 className="text-2xl font-bold italic">ทะเบียนรถเลขตัวเดียว</h2>
      </div>
...
export default PlatesSingleDigit;
...
const PlatesNewNonAuction = ({ cover, url }) => {
  const [platesNewNonAuction, setPlatesNewNonAuction] = useState([]);

  // ✅ ดึงข้อมูลทะเบียนรถหมวดใหม่ที่ไม่ใช่ป้ายประมูลจาก API
  useEffect(() => {
    const fetchPlatesNewNonAuction = async () => {
      try {
        const response = await fetch(`${url}/plates_new_non_auction`);
        if (!response.ok) throw new Error("Failed to fetch plates");
        const data = await response.json();
        setPlatesNewNonAuction(data);
      } catch (error) {
        console.error("❌ Error fetching plates_new_non_auction:", error);
      }
    };
    fetchPlatesNewNonAuction();
  }, [url]);
  ...
  return (
    <>
      ...
      <div className="text-center mt-12 mb-6">
        <h2 className="text-2xl font-bold italic">ทะเบียนรถหมวดใหม่ (ไม่ใช่ประมูล)</h2>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
        {platesNewNonAuction.length > 0 ? (
          ...
          <p className="text-center text-gray-400 col-span-4">
            ❌ ไม่พบทะเบียนรถหมวดใหม่ที่ไม่ใช่ป้ายประมูล
          </p>
        )}
      </div>
    </>
...
export default PlatesNewNonAuction;